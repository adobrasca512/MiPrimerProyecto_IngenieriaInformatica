		...
		btnEntrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Micuenta cuenta= new Micuenta(window2.nombres, "", "", "", "", 0, 0, "");
         		...
         		while(!encontrado && i<matriz[0].length ) {
         			if(txtUsuario.getText().equals(matriz[i][0])) {
         				...
         				if(passwordField.getText().equals(matriz[i][1])) {
        					...
        					//	DatosCliente datos= new DatosCliente(window2.nombres, window2.apellidos,window2.correos, window2.identificacion,window2.ciudades, tel, cp, window2.paises);
         					...
         					String matrizalmacen[][] = prueba.getLectura_especial_almacen(rutas);  
         					Comprascliente compras = new Comprascliente(prueba, "", "", 0, 0,matrizalmacen,rutas);
        						JOptionPane.showMessageDialog(null,"Bienvenido al Sistema!");
        						
        						compras.Menu_Clientes(compras.getDesicion(), compras.getMatriz());
        						//estaventana.dispose();
        					//data.getDardeAlta();
